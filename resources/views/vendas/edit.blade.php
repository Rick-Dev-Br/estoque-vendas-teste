                                ...
                                <div class="row mb-2 produto-item">
                                    <div class="col-md-6">
                                        <select class="form-select produto-select"
                                            name="itens[{{ $loop->index }}][produto_id]" required>
                                            <option value="">Selecione um produto</option>
                                            @foreach($produtos as $produto)
                                                @php
                                                    $selecionado = (int)($item['produto_id'] ?? 0) === $produto->id;
                                                    $indisponivel = $produto->status !== 'ativo' || $produto->estoque <= 0;
                                                @endphp
                                                <option value="{{ $produto->id }}"
                                                    data-preco="{{ $produto->preco }}"
                                                    {{ $selecionado ? 'selected' : '' }}
                                                    {{ (!$selecionado && $indisponivel) ? 'disabled' : '' }}>
                                    ...
                                    <div class="col-md-3">
                                        <input type="number" class="form-control quantidade"
                                            name="itens[{{ $loop->index }}][quantidade]" min="1"
